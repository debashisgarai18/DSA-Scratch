// to insert element at kth pos in the SLL
...
// function to insert the node in the Kth posiiton
Node* insKPos(Node* head, int k, int ele){
  if(head == NULL) return new Node(ele);
  // to check if the k = 1  or not
  if(k == 1){
  ...
  while(temp){
    ...
    cnt++;
    // just check for the pos of the node before the given k and create the node
    // if the count is matching the val of k - 1
    if(cnt == k - 1){
      // once the node is created then jus link that with the next node and link the node node prev to this node with the newNode
